...
class accederAFunciones implements Instruccion {
    ...
    ejecutar(entorno: Entorno): Object {
        ...
        if (nodo.tipo === Tipo.ID) {
            ...
            let visibilidad: Visibilidad = f.atributos['visibilidad'] as Visibilidad;
            if (visibilidad !== Visibilidad.PUBLIC) {
                let mensaje: MensajeError = new MensajeError("Semantico", "El metodo tiene una visibilidad, " + Visibilidad[visibilidad], entorno.archivo, this.l, this.c);
                Auxiliar.agregarError(mensaje);
                return mensaje;
            }
            ...
            let isStatic:Boolean = f.atributos['isStatic'] as Boolean;
            if(!isStatic){
                let mensaje: MensajeError = new MensajeError("Semantico", "El metodo tiene que ser static", entorno.archivo, this.l, this.c);
                Auxiliar.agregarError(mensaje);
                return mensaje;
            }
